Shader "Hidden/ElipseShader"
{
    Properties
    {
        _MainTex ("Texture", 2D) = "white" {}
        [HideInInspector]_SliderX ("Center X", Range(0, 1)) = 0.5
        [HideInInspector]_SliderY ("Center Y", Range(0, 1)) = 0.5
        ...
        Pass
        {
            ...
            fixed4 frag (v2f i) : SV_Target
            {
                ...
                if (!isInsideEllipse(adjustedCenter, adjustedRadii, adjustedUV))
                {
                    ...
                    fixed4 pixelColor = tex2D(_MainTex, pixelatedUV);

                    fixed4 gradientColor = _GradientColor;
                    gradientColor.a = _Transparency;

                    fixed4 colorWithGradient = gradientColor * uv.x;

                    return lerp(pixelColor, colorWithGradient, _Transparency);
                }
